...
def find_existing_observation(db: Session, acquisition_timestamp: datetime, acq_system_id: int, acq_num_1: int, acq_num_2: int) -> Optional[Observation]:
    return db.query(Observation).filter_by(
        acq_timestamp=acquisition_timestamp,
...
def build_observation_fields(obs_row: dict, obs_details: dict) -> dict:
    ...
    nat_key, disp_name = obs_key(obs_row["AcqSystemID"], obs_row["AcqTimestamp"], obs_row["AcqNum1"], obs_row["AcqNum2"])
    return dict(
    ...
    with get_record_db() as db:
        ...
        if db_record is not None and not force_ingest:
            if db_record.filesize == filesize and db_record.last_file_update == last_file_update:
                logger.info("[OBS] Found an existing record and it has not changed. Moving on.")
        ...
        logger.debug(f"Found {len(obs_rows)} observations in database")
        obs_rows = obs_rows[unique_idxs]
        logger.debug(f"Unique rows: {len(obs_rows)}")
        ...
        for obs_row, sequence_count in zip(obs_rows,sequence_counts):
            ...
            observation = find_existing_observation(
                db, fields["acq_timestamp"], fields["acq_system_id"], fields["acq_num_1"], fields["acq_num_2"]
            )
...
def ingest_md_db(metadata_db_path,logger):
    ...
    if not exists(schedule_path):
        schedule_path = None
    logger.info(f"[OBS] Ingesting {metadata_db_path}")
    with MetadataDB(metadata_db_path) as target_db:
        target_dat = MetadataDat(dat_path)
        _ingest_md_db(target_db, target_dat, data_dir, schedule_path=schedule_path,logger=logger)
        logger.info('[OBS] Ingested.')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Recursively ingest observations in Metadata.db files into the records database")
    ...
    for metadata_db_path in tqdm(db_paths, desc="Ingesting metadata dbs"):
        ingest_md_db(metadata_db_path, logger)
