...
import { getResultsDBs, getResultsDBsCount } from "@api/results_db";
...
export function ResultsDBDisplay({ title = "Results Databases" }: { title?: string }) {
    ...
    const [sortIndex, setSortIndex] = usePersistedSort("results_db_sort", SORT_PARAMS);
    ...
    const { data: matchingCount } = getResultsDBsCount(params);
    const { data: totalCount } = getResultsDBsCount();
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
            <Stack direction="row" spacing={2} sx={{ alignItems: 'center', flexGrow: 14 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', alignSelf: 'stretch' }}>
                    <FilteredCollectionLengthChip matching={matchingCount ?? 0} total={totalCount ?? 0} tooltip="Results databases matching search / total results databases" />
                </Box>
            </Stack>
            {controls}
        </Box>
